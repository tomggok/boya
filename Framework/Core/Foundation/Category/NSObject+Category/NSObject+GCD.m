...
void (^independentBlockObject)(void)=^(void)
{
    ...
    NSLog(@"localInteger==%ld",(long)localInteger);
};
...
-(void)simpleMethod
{
    ...
    [array sortUsingComparator:
     ^NSComparisonResult(id obj1,id obj2){
         ...
         NSLog(@"outsideVariable==%lu",(unsigned long)outsideVariable);//
         NSLog(@"insideVariable==%lu",(unsigned long)insideVariable);

         NSLog(@"self==%@",self);//simpleMethod方法是OC类的实例,所以此方法里的内联闭包也可以获取self
...
+(void)scopeTest
{
    ...
    BlockWithNoParams myBlock=^{
        NSLog(@"integerValue inside the block=%lu",(unsigned long)integerValue); //如果变量的声明时前边没加__block,则在此内联闭包执行时复制了一个integerValue 的只读值,所以下边调myBlock()方法时,打印结果还是10,否则打印20;
    };

    integerValue=20;
    myBlock();
    NSLog(@"integerValue outside the block =%lu",(unsigned long) integerValue);
}
...
+(void)dispatch_asyncOnMainQueue
{
    ...
    dispatch_async(mainQueue, /*内联闭包*/^(void){
        ...
        {//分配到主队列的闭包实际在主线程上运行
            NSLog(@"Current thread=%@",[NSThread currentThread]);//当前线程
            NSLog(@"Main thread=%@",[NSThread mainThread]);//主线程
        }
...
+(void)dispatch_after
{
    ...
    dispatch_queue_t concurrentQueue=dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0);
        ...
        //    NSLog(@"concurrentQueue==%p",concurrentQueue);
        //    NSLog(@"dispatch_get_main_queue()==%p",dispatch_get_main_queue());
        //
        //    NSLog(@"Current thread=%@",[NSThread currentThread]);//当前线程
        //    NSLog(@"Main thread=%@",[NSThread mainThread]);//主线程
...
+(void)firstCounter
{
    for (NSUInteger counter=0;counter<10; counter++) {
        NSLog(@"firstCounter=%lu",(unsigned long)counter);
    }
}
...
+(void)secondCounter
{
    @autoreleasepool {//
        for (NSUInteger counter=0;counter<10; counter++) {
            NSLog(@"secondCounter=%lu",(unsigned long)counter);
        }
    }
