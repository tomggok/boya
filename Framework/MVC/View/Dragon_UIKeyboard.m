...
#import "Dragon_UIKeyboard.h"

@interface DragonUIKeyboard ()
{
...
@implementation DragonUIKeyboard
...
static DragonUIKeyboard *sharedInstace = nil;
+ (DragonUIKeyboard *)sharedInstace
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstace = [[DragonUIKeyboard alloc] init];
    });
...
- (void)handleNotification:(NSNotification *)notification
{
    ...
    if ([notification is:UIKeyboardDidShowNotification])
    {
        ...
        {
            _shown = YES;
            [self postNotification:[DragonUIKeyboard SHOWN]];
        }
        ...
        if (value)
        {
            ...
            if (keyboardHeight != _height)
            {
                ...
                [self postNotification:[DragonUIKeyboard HEIGHT_CHANGED]];
                animted = NO;
            }
    ...
    }else if ([notification is:UIKeyboardWillChangeFrameNotification])
    {
        ...
        if (value1 && value2)
        {
            ...
            }else if (rect2.origin.y >= [UIScreen mainScreen].bounds.size.height)
            {
                ...
                {
                    _shown = NO;
                    [self postNotification:[DragonUIKeyboard HIDDEN]];
                }
            }
        }
