...
@implementation BoyaSIMenuButton
...
//        self.arrow = [[UIImageView alloc] initWithImage:[BoyaSIMenuConfiguration arrowImage]];
//        [self addSubview:self.arrow];
    }
    return self;
}

-(void)initArrowImage:(UIImage *)img
{
    self.arrow = [[UIImageView alloc] initWithImage:img];
    [self addSubview:self.arrow];
    [self.arrow setBounds:CGRectMake(0, 0, img.size.width/2, img.size.height/2)];
    [self.arrow release];
}
...
- (void)layoutSubviews
{
    ...
    self.arrow.center = CGPointMake(CGRectGetMaxX(self.title.frame) + [BoyaSIMenuConfiguration arrowPadding], self.frame.size.height / 2);
}
...
{
    if (((BoyaSINavigationMenuView *)(self.superview)).table._b_isAnimating) {//避免列表正在动画时点击BT又生成新列表
        return NO;
    }
